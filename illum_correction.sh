...
echo Beginn Katalog erstellen: `date` >> ~/dauer.txt
...
do
  ...
  ZP=`${P_GAWK} 'NR == '${SOLUTION}' {print $1+$2+$3}' ${MAIND}/${STANDARDD}/calib/night_${NIGHT}_${FILTERNAME}_result.asc`
  i=1
  while [ ${i} -le ${NCHIPS} ]
  do
  ...
  do
    ...
    ${P_LDACTOASC} -b -i ${TEMPDIR}/chip_${i}_merg_corr.cat -t PSSC \
	    ...
	    Ypos_mod AIRMASS Xpos_global Ypos_global >> ${MAIND}/${STANDARDD}/calib/residuals_${NIGHT}/chip_${i}.csv
  ...
  echo Ende Katalog erstellen: `date` >> ~/dauer.txt
  echo Beginn Fit erstellen: `date` >> ~/dauer.txt

  # Fitting the data
  ./illum_correction_fit.py ${MAIND}/${STANDARDD}/calib/residuals_${NIGHT}/

  echo Ende Fit erstellen: `date` >> ~/dauer.txt
  echo Begin Fit anwenden: `date` >> ~/dauer.txt
  ...
  do
    ...
    ${P_LDACTOASC} -b -i ${TEMPDIR}/chip_${i}_merg_fitted.cat -t PSSC -k Xpos Ypos Mag MagErr ${FILTER}mag IMAGEID \
      Residual Xpos_mod Ypos_mod Mag_fitted Residual_fitted AIRMASS Xpos_global Ypos_global >> ${MAIND}/${STANDARDD}/calib/residuals_${NIGHT}/chip_${i}.csv
    cp ${TEMPDIR}/chip_${i}_merg_fitted.cat ${MAIND}/${STANDARDD}/calib/residuals_${NIGHT}/ &
    i=$(( $i + 1 ))
  done

  echo Ende Fit anwenden: `date` >> ~/dauer.txt
  echo Begin plot fitted: `date` >> ~/dauer.txt
  ...
  echo Ende plot fitted: `date` >> ~/dauer.txt
  echo Begin Statistiken berechnen: `date` >> ~/dauer.txt
  ...
  echo Ende Statistiken berechnen: `date` >> ~/dauer.txt
  
  # Creating a contour plot from the correction function and create a correction FITS file...
  ./illum_correction_contourplot_fitfunction.py ${MAIND}/${STANDARDD}/calib/residuals_${NIGHT}/
done


# Cleaning up...
rm ${TEMPDIR}/chip_*_merg_corr*.cat ${TEMPDIR}/chip_*_merg_fitted*.cat ${TEMDIR}/res_*.csv &
echo Ende: `date` >> ~/dauer.txt
echo "" >> ~/dauer.txt
